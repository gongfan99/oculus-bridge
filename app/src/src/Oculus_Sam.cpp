...
{
    return 0.064;//mStereoConfig.GetIPD();
}

float Oculus::getFov()
{
    return 50.0;//mStereoConfig.GetYFOVDegrees();
}

float Oculus::getEyeToScreenDistance() const
{
    return 0.041;//mStereoConfig.GetEyeToScreenDistance();
}

float Oculus::getProjectionCenterOffset()
{
    return 0.064;//mStereoConfig.GetProjectionCenterOffset();
}

float Oculus::getDistortionScale()
{
    return 1.0;//mStereoConfig.GetDistortionScale();
}

Vec4f Oculus::getDistortionParams() const
{
    return Vec4f( 1.0, 0.22, 0.24, 0.0 );
...
ci::Quatf Oculus::getOrientation()
{
	...
	theta += 0.1*3.1416/180.0; //rotate 0.1 degree per frame
    return ci::Quatf( cos(theta/2), 0.0*sin(theta/2), 0.0*sin(theta/2), 1.0*sin(theta/2) );
